...
function chord(){
  ...
  d3.csv("../degrees.csv").then((data) => {
    ...
    // combines arr1 and arr2 into 1
    let names = arr1.concat(arr2);


    names = [...new Set(names)]; //spread syntax; this gives us unique values
    console.log(names)
    ...
    // map gives positions to count on
    ...
    svg.append("g")
      .selectAll("g") // select all groups before adding data to group
      .data(chords)
      .join("path")
      .attr("d", ribbon) // just an attribute; gives coordinates of how to draw and change positions
